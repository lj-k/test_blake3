...
ifeq ($(PLT),AVX512)
	...
	BLAKE3_PLATFORM_FILE=blake3_avx512.c
else ifeq ($(PLT),RVV)
	C_DEF_PLATFORM=-DTEST_USE_RVV=1
	GCC_FLAGS=-o $(EXE_PATH)test_$(PLT)_$(MNUM)
	BLAKE3_PLATFORM_FILE=blake3_RVV.c
else ifeq ($(PLT),NEON)
	C_DEF_PLATFORM=-DTEST_USE_NEON=1
	GCC_FLAGS=-o $(EXE_PATH)test_$(PLT)_$(MNUM)
	BLAKE3_PLATFORM_FILE=blake3_neon.c
else ifeq ($(PLT),PORT)
	C_DEF_PLATFORM=-DTEST_USE_PORTABLE=1
	GCC_FLAGS=-o $(EXE_PATH)test_$(PLT)_$(MNUM)
	BLAKE3_PLATFORM_FILE=blake3_portable.c
else 
	PLT=PORT
	C_DEF_PLATFORM=-DTEST_USE_PORTABLE=1
	GCC_FLAGS=-o $(EXE_PATH)test_$(PLT)_$(MNUM)
...
TEST_MANY_FILES=$(SRC_PATH)perftest_module_many.c $(SRC_PATH)$(BLAKE3_PLATFORM_FILE)
TEST_MANY_DEFINES=-DUSEGCCDF $(C_DEF_MANY_NUM) $(C_DEF_SWITCH_PRINT) $(C_DEF_SWITCH_TIMETEST)$(C_DEF_PLATFORM)

TEST_XOF_FILES=$(SRC_PATH)perftest_module_xofinp.c $(SRC_PATH)$(BLAKE3_PLATFORM_FILE)
TEST_XOF_DEFINES=-DUSEGCCDF -DTEST_XOF=1 $(C_DEF_SWITCH_PRINT) $(C_DEF_SWITCH_TIMETEST)

TEST_INP_FILES=$(SRC_PATH)perftest_module_xofinp.c $(SRC_PATH)$(BLAKE3_PLATFORM_FILE)
